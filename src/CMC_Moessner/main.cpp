...
void UsageHelp()
{
	cout << "--> Set Lozenge lattice simulations program" << endl << endl;

	cout << "    Usage: ./Set_Moessner_Lab [input file]" << endl << endl;

	cout << "  > Optional arguments" << endl;
	cout << "       [input file]:  path to the file containing the parameters" << endl;
	cout << "                      Default value = \"params_Moessner.in\"" << endl << endl;
}
...
int main(int argc, char **argv)
{
	...
	// ---> Geometry parameters
	string SimType = "Moessner";
	SysConf conf(input,SimType);
	conf.SetNeighboursMoessner();
	conf.SetInitialMoessner();
	conf.SetPositions();
	conf.SetDistances();
	...
	dummyString = "./" + outFolder + "/SqrDistances_Key.dat";
	ofstream	distanceFile(dummyString.c_str(),ios::trunc);
	for(int iii = 0; iii < conf.nbOfDists; ++iii)
	{
		distanceFile << conf.SqrDistancesKey[iii] << " ";
	}
	distanceFile.close();

	dummyString = "./" + outFolder + "/All_SqrDistances.dat";
	distanceFile.open(dummyString.c_str(),ios::trunc);
	for(int iii = 0; iii < conf.L; ++iii)
	{
		for(int jjj = 0; jjj < conf.L; ++jjj)
		{
			distanceFile << conf.SqrDistancesTable[idxConv(conf.L,iii,jjj)] << " ";
		}
		distanceFile << endl;
	}
	distanceFile.close();
...
//		ExportObject(dummyOrder,dummyStringBis.c_str());
	...
	//    ...
	//
	string scriptFilename	= "./run_Moessner_" + ToString(pid) + ".sh";
	string commandsFilename	= "comm_Moessner_" + ToString(pid) + ".sh";
